...
std::vector<int> Merge(const std::vector<int>& a, const std::vector<int>& b) {
    std::vector<int> c;
    int i = 0;  // Índice para recorrer el vector a
    int j = 0;  // Índice para recorrer el vector b
    ...
    // Agregar los elementos restantes de a (si los hay)
    while (i < a.size()) {
        c.push_back(a[i]);
        i++;
    }

    // Agregar los elementos restantes de b (si los hay)
    while (j < b.size()) {
        c.push_back(b[j]);
        j++;
    }

    return c;  // En c obtengo la mezcla de "a" y "b" ordenada
}
